<template>
  <div>
    <v-card flat class="">
      <v-card-title class="title d-flex justify-center" primary-title>
        <h1 class="grey--text">
        ...
        <v-container class="my-5 mx-auto pa-0">
          ...
          >
            <v-layout
              row
              wrap
              :class="`ma-auto pa-3 project ${project.status}`"
            >
              <v-flex xs6 sm4 md5>
              ...
              <v-flex xs12 sm12 md2>
                <div class="d-flex justify-center algin-center mt-5">
                  <v-chip
                    id="chip"
                    :class="`white--text ${project.status} caption`"
                    >{{ project.status }}
                  </v-chip>
                </div>
              </v-flex>
              ...
              <v-flex xs12 class="d-flex justify-center ma-3">
                <v-btn
                  ...
                  :loading="loading"
                >
                  <span :class="{ 'success-text': !project.done }">
                    &#x2714;
                  </span>
                </v-btn>
                <v-btn
  ...
  methods: {
    ...
    submit() {
      if (this.$refs.form.validate()) {
        ...
        addDoc(collection(db, getAuth().currentUser.uid), {
          title: this.title,
          person: this.person,
      ...
      for (let i = 0; i < this.projects.length; i++) {
        if (this.projects[i].id === elementId) {
          onAuthStateChanged(getAuth(), (user) => {
            updateDoc(doc(db, user.uid, elementId), {
              status: this.projects[i].status,
              done: this.projects[i].done,
              count: this.projects[i].count,
            });
          });
        }
      }
      ...
      for (let i = 0; i < this.projects.length; i++) {
        if (this.projects[i].dueBy < this.currentDate) {
          if (this.projects[i].status === "complete") {
            ...
            console.log(
              "project status on checkDate" + this.projects[i].status
            );
            onAuthStateChanged(getAuth(), (user) => {
              updateDoc(doc(db, user.uid, this.projects[i].id), {
                status: this.projects[i].status,
              });
            });
          }
    ...
    deleteItem(elementId) {
      for (let i = 0; i < this.projects.length; i++) {
        if (this.projects[i].id === elementId) {
          ...
          onAuthStateChanged(getAuth(), (user) => {
            deleteDoc(doc(db, user.uid, elementId));
          });
        }
      }
    },
